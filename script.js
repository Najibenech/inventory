...
if (loginForm) {
  loginForm.addEventListener('submit', (e) => {
    ...
    localStorage.setItem('ac_current', JSON.stringify({ username: user.username, branch: user.branch || '', role: user.role || '' }));
      ...
      setTimeout(() => { window.location.href = 'ardour_cafe_inventory_system/inventory.html'; }, 400);
    } else {
...
function recordOutgoing(branchKey, data) {
  ...
  // save outgoing to branch history
...
function editOutgoing(branchKey, index) {
  // legacy: use modal-based edit now
...
function closeEditModal() {
  const modal = qs('#editModal'); if (!modal) return; modal.classList.add('hidden'); modal.setAttribute('aria-hidden','true');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const deleteBtn = qs('#editDeleteBtn'); if (deleteBtn) deleteBtn.addEventListener('click', () => {
    ...
    deleteOutgoing(branchKey, idx); closeEditModal();
  });
});
...
function deleteStock(index) {
  ...
  // subtract from store if possible
  const items = getStore();
  const it = items.find(i => i.name.toLowerCase() === rec.name.toLowerCase() && i.units === rec.units);
  if (it) {
  ...
  hist.splice(index,1); localStorage.setItem('ac_stock', JSON.stringify(hist)); refreshStockTable(); refreshStoreTable(); populateSelects();
}
...
function parseCsv(text) {
  // very small CSV parser for well-formed quoted CSV
...
document.addEventListener('DOMContentLoaded', () => {
  // display current user in header if present
  ...
  const marinaForm = qs('#marinaForm'); if (marinaForm) {
    marinaForm.addEventListener('submit', (e) => {
      ...
      // small timeout to show processing state (keeps UX responsive without async back-end)
      setTimeout(() => {
  ...
  const impStore = qs('#importStore'); if (impStore) impStore.addEventListener('change', (e) => { const f = e.target.files && e.target.files[0]; if (f) importCsvFile(f, (rows) => { // skip header
    rows.slice(1).forEach(cols => { const [name,category,units,quantity] = cols; const items = getStore(); const existing = items.find(i=>i.name===name && i.units===units); if (existing) existing.quantity = parseInt(existing.quantity,10) + parseInt(quantity||0,10); else items.push({ name, category, units, quantity: parseInt(quantity||0,10) }); saveStore(items); }); refreshStoreTable(); populateSelects(); e.target.value = ''; }); });
